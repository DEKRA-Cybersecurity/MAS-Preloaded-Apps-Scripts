...
def insert_new_total_fail_counts(tfc_data):
    ...
    query = """INSERT INTO Total_Fail_Counts (HASH, ID_EXECUTION, CODE_1, CODE_2, CRYPTO_1, CRYPTO_3, NETWORK_1, NETWORK_2, NETWORK_3, PLATFORM_2, PLATFORM_3, STORAGE_2) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""

    try:
        cursor.execute(query, tuple(tfc_data))
        cnx.commit()
    except Exception as e:
        print(tfc_data)
        print(f'FAIL: {e}')
...
def get_values_permissions(uuid_execution):
    ...
    query = "SELECT * FROM Permissions WHERE ID_EXECUTION = '" + str(uuid_execution) + "'"
