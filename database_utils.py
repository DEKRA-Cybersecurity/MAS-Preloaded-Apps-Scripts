...
def first_execution():
    cnx = mysql.connector.connect(user='jmsl', password='mysqlpassword')
    cursor = cnx.cursor()
...
def insert_values_total_fail_count(apk_hash, case, total_fail_count):
    cnx = mysql.connector.connect(user='jmsl', password='mysqlpassword')
    cursor = cnx.cursor()
    cursor.execute('USE automated_MASA')

    query = f"""INSERT INTO Total_Fail_Counts (HASH, {case}) VALUES (%s, %s)"""
    
    try:    
        cursor.execute(query, (apk_hash, total_fail_count))
        cnx.commit()
        print("Success")
        return "success"
    except:
